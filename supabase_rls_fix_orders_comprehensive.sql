...
DROP POLICY IF EXISTS "Allow anon select for orders" ON public.orders; -- Remova a política de negação anterior se ela foi criada
...
-- A condição WITH CHECK garante que o 'store_id' seja sempre preenchido.
-- Usamos 'store_id IS NOT NULL' para garantir que o pedido está vinculado a uma loja.
...
-- seus próprios pedidos, vinculados ao seu 'store_id'.
